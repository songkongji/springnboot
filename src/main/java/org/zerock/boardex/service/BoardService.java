...
//원래 인터페이스에서는 메서드를 정의만 하고 내용을 구현하는 것은 자식 객체(Impl)에서 햇었음.
//    인터페이스에서 선언한 메서드는 반드시 자식 객체(Impl)에서 반드시 구현해야 함.
//    디폴트 메서드를 구현하여 그대로 자식객체로 상속해 줄 수 있음.
...
//    Board엔티티 객체를 BoardDTO 타입으로 변환하는 default메서드
    default BoardDTO entityToDTO(Board board){
        BoardDTO boardDTO = BoardDTO.builder()
                ...
                .build();
//        첨부파일의 Uuid + "_" + 파일명 합친 문자열을 fileName에 배열로 저장
