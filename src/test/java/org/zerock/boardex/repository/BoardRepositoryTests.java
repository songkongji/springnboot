...
public class BoardRepositoryTests {
    ...
    public void testSearchReplyCount() {
        ...
        String keyword = "1";
        Pageable pageable = PageRequest.of(0, 10, Sort.by("bno").descending());
        Page<BoardListReplyCountDTO>
                result = boardRepository.searchWithReplyCount(types, keyword, pageable);
//        전체 페이지
        log.info(result.getTotalPages());
        log.info(result.getSize());
        log.info(result.getNumber());
...
//    이미지 로딩(게시물 조회) 테스트
    @Test
//    필요할 때마다 메서드 내에서 추가적인 쿼리를 여러번 실행할 수 있다.
    @Transactional
...
//        boardRepository의 findByIdWithImage 메서드 호출
